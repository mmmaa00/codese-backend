...
const createProduct = async ({display, provider,description, imageUrl,priceIn, priceOut, priceSale,shipday, instock, status, categoryId }) => {
  const sql = 
    ...
    VALUES(uuid(),?,?,?,?,?,?,?,?,?,?,?);`
  const data = await db.query(sql,[display, provider, description, imageUrl, priceIn, priceOut, priceSale, shipday, instock, status, categoryId]);
}
const updateProductById = async (productId,{display, provider, description, imageUrl, priceIn, priceOut, priceSale, shipday, instock, status, categoryId }) => {
  ...
  WHERE productId = ? 
  AND isDelete = 0;`;
  const data = await db.query(sql,[display, provider, description, imageUrl, priceIn, priceOut, priceSale, shipday, instock, status, categoryId, productId])
};
